...
#define TR_LEN	1024
uint16_t RX_Buffer[TR_LEN] = {0};
uint16_t TX_Buffer[TR_LEN] = {
...
int main(void)
{
	...
	while(1==1)
	{
		if(RX_DoneIndex)
		{
			int offset = (RX_DoneIndex & 1) ? TR_LEN/2 : 0;
...
//	PORT_Init(PORTM, PIN4, PORTM_PIN4_SPI0_MISO, 1);	// I2S_DI
	...
	DMA_initStruct.INTEn = DMA_IT_DST_PART | DMA_IT_DONE;
...
//	PORT_Init(PORTB, PIN3, PORTB_PIN3_SPI1_MISO, 0);	// I2S_DO
	...
	DMA_initStruct.Handshake = DMA_CH2_SPI1RX;
	DMA_initStruct.Priority = DMA_PRI_LOW;
	DMA_initStruct.INTEn = DMA_IT_DST_PART | DMA_IT_DONE;
...
void DMA_Handler(void)
{
	...
	/* Slave RX */
	if(DMA_CH_INTStat(DMA_CH2, DMA_IT_DST_PART))
	{
		DMA_CH_INTClr(DMA_CH2, DMA_IT_DST_PART);
...
int fputc(int ch, FILE *f)
{
	...
	*((volatile uint32_t *) 0xCC000000) = ch;
