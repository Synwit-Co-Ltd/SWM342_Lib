...
* 函数名称: DMA_CH_Init()
* 功能说明:	DMA通道初始化
* 输    入: uint32_t chn			指定要配置的通道，有效值有DMA_CH0、DMA_CH1、DMA_CH2、DMA_CH3、DMA_CH4、DMA_CH5
...
void DMA_CH_Init(uint32_t chn, DMA_InitStructure * initStruct)
{
	...
	DMA->CH[chn].PARTLEN = initStruct->Count / 2;
...
/****************************************************************************************************************************************** 
* 函数名称: DMA_CH_Abort()
* 功能说明:	提前终止传输，并将 DMA FIFO 的数据 Flush 到 RAM
* 输    入: uint32_t chn			指定要配置的通道，有效值有DMA_CH0、DMA_CH1、DMA_CH2、DMA_CH3、DMA_CH4、DMA_CH5
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
{
	DMA->CH[chn].CR |= DMA_CR_ABORT_Msk;
}
...
* 功能说明:	DMA中断使能
* 输    入: uint32_t chn			指定要配置的通道，有效值有DMA_CH0、DMA_CH1、DMA_CH2、DMA_CH3、DMA_CH4、DMA_CH5
*			uint32_t it				interrupt type，有效值有 DMA_IT_DONE、DMA_IT_DST_PART、DMA_IT_SRC_PART
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
{
	DMA->IM &= ~(it << chn);
	DMA->DSTIM &= ~((it >>  8) << (chn * 3));
	DMA->SRCIM &= ~((it >> 16) << (chn * 3));
}
...
{
	DMA->IF = (it << chn);
	DMA->DSTIF = ((it >>  8) << (chn * 3));
	DMA->SRCIF = ((it >> 16) << (chn * 3));
}
