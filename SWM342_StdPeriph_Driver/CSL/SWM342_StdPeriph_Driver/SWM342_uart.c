...
* 函数名称:	UART_Init()
* 功能说明:	UART串口初始化
* 输    入: UART_TypeDef * UARTx	指定要被设置的UART串口，有效值包括UART0、UART1、UART2、UART3、UART4、UART5
...
{	
	...
	case ((uint32_t)UART5):
		SYS->CLKEN1 |= (0x01 << SYS_CLKEN1_UART5_Pos);
		break;
	}
	
	UART_Close(UARTx);	//一些关键寄存器只能在串口关闭时设置
	
	int div = ((UARTx == UART4) || (UARTx == UART5)) ? 2 : 1;
	
	UARTx->BAUD &= ~(UART_BAUD_BAUD_Msk | UART_BAUD_FRAC_Msk);
	UARTx->BAUD |= (((SystemCoreClock/div/initStruct->Baudrate - 1) / 16) << UART_BAUD_BAUD_Pos) |
				   (((SystemCoreClock/div/initStruct->Baudrate - 1) % 16) << UART_BAUD_FRAC_Pos);
	...
	if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn)
	{
		switch((uint32_t)UARTx)
		{
		case ((uint32_t)UART0):	NVIC_EnableIRQ(UART0_IRQn); break;
		case ((uint32_t)UART1):	NVIC_EnableIRQ(UART1_IRQn); break;
		case ((uint32_t)UART2):	NVIC_EnableIRQ(UART2_IRQn); break;
		case ((uint32_t)UART3):	NVIC_EnableIRQ(UART3_IRQn); break;
...
* 函数名称:	UART_WriteByte()
* 功能说明:	发送一个字节数据
* 输    入: UART_TypeDef * UARTx	指定要被设置的UART串口，可取值包括UART0、UART1、UART2、UART3、UART4、UART5、UART4
*			uint8_t data			要发送的字节			
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
void UART_SetBaudrate(UART_TypeDef * UARTx, uint32_t baudrate)
{
	...
	UARTx->BAUD &= ~(UART_BAUD_BAUD_Msk | UART_BAUD_FRAC_Msk);
	UARTx->BAUD |= (((SystemCoreClock/div/baudrate - 1) / 16) << UART_BAUD_BAUD_Pos) |
				   (((SystemCoreClock/div/baudrate - 1) % 16) << UART_BAUD_FRAC_Pos);
}
...
uint32_t UART_GetBaudrate(UART_TypeDef * UARTx)
{
	...
	return SystemCoreClock/div/(((UARTx->BAUD & UART_BAUD_BAUD_Msk) >> UART_BAUD_BAUD_Pos) * 16 +
