...
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1、SPI2
...
void SPI_Init(SPI_TypeDef * SPIx, SPI_InitStructure * initStruct)
{
	...
	case ((uint32_t)SPI2):
		SYS->CLKEN1 |= (0x01 << SYS_CLKEN1_SPI2_Pos);
		break;
	}
	...
	if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TXCompleteIEn)
	{
		switch((uint32_t)SPIx)
		{
		case ((uint32_t)SPI0): NVIC_EnableIRQ(SPI0_IRQn);  break;
		case ((uint32_t)SPI1): NVIC_EnableIRQ(SPI1_IRQn);  break;
		case ((uint32_t)SPI2): NVIC_EnableIRQ(SPI2_IRQn);  break;
		}
	}
...
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1、SPI2、SPI1
*			uint32_t				要写入的数据
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
* 注意事项: 无
******************************************************************************************************************************************/
void SPI_BIDI_In(SPI_TypeDef * SPIx)
{
	uint32_t reg = SPIx->CTRL;
	
	reg &= ~(SPI_CTRL_BIDIMODE_Msk | SPI_CTRL_BIDIOE_Msk);
	reg |= (1u<< SPI_CTRL_BIDIMODE_Pos) | (0 << SPI_CTRL_BIDIOE_Pos);
...
* 注意事项: 无
******************************************************************************************************************************************/
void SPI_BIDI_Out(SPI_TypeDef * SPIx)
{
	uint32_t reg = SPIx->CTRL;
	
	reg &= ~(SPI_CTRL_BIDIMODE_Msk | SPI_CTRL_BIDIOE_Msk);
	reg |= (1u<< SPI_CTRL_BIDIMODE_Pos) | (1 << SPI_CTRL_BIDIOE_Pos);
...
* 注意事项: 无
******************************************************************************************************************************************/
void SPI_BIDI_Off(SPI_TypeDef * SPIx)
{
	uint32_t reg = SPIx->CTRL;
