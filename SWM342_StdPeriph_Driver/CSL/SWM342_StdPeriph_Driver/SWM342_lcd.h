#ifndef __SWM342_LCD_H__
#define __SWM342_LCD_H__


typedef struct {
	uint8_t  ClkDiv;		//取值范围2--65
	uint8_t  Format;		//LCD_FMT_RGB565、LCD_FMT_RGB888、LCD_FMT_SRGB565、LCD_FMT_SRGB888
	uint16_t HnPixel;		//水平方向像素个数，最大取值8192
	uint16_t VnPixel;		//垂直方向像素个数，最大取值8192
	uint16_t Hfp;			//horizonal front porch，最大取值512
	uint16_t Hbp;			//horizonal back porch， 最大取值512
	uint16_t Vfp;			//vertical front porch， 最大取值512
	uint16_t Vbp;			//vertical back porch，  最大取值512
	uint16_t HsyncWidth;	//HSYNC低电平持续多少个DOTCLK，最大取值512
	uint16_t VsyncWidth;	//VSYNC低电平持续多少个行时间，最大取值512
