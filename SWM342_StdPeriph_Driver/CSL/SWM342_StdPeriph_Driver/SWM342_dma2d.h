#ifndef __SWM342_DMA2D_H__
#define __SWM342_DMA2D_H__

#include <stdint.h>


typedef struct {
	uint16_t Interval;			// 每传输一块数据（默认32个字），等待指定个系统周期后再传输下一个块，防止DMA2D占用过多SDRAM带宽，影响LCD读取显示数据；取值1--65536
...
typedef struct {
	...
	uint8_t  AlphaMode;			// DMA2D_AMODE_PIXEL、DMA2D_AMODE_ALPHA、DMA2D_AMODE_PMULA、DMA2D_AMODE_EXTERN、...
	uint8_t  Alpha;				// 用于 DMA2D_AMODE_ALPHA 模式，为图层指定一个固定的 Alpha 值
	uint32_t AlhpaAddr;			// 用于 DMA2D_AMODE_EXTERN 模式，只能用于前景层，Alpha值存储在一个由 AlphaAddr 指定的单独的存储区
...
#define DMA2D_AMODE_EXTERN	(0 | (3 << 5))	// 只能用于前景层，Alpha值存储在一个由 AlphaAddr 指定的单独的存储区
