...
uint16_t *Img_Buffer = (uint16_t *)(SDRAMM_BASE + 0x100000);	// SRA 输入图片
uint16_t *SRA_Buffer = (uint16_t *)(SDRAMM_BASE + 0x180000);	// SRA 变换结果
...
void test_SRA(uint16_t img_width, uint16_t img_height, uint16_t scl_width, uint16_t scl_height, uint32_t roate_angle)
{
	...
	SRA_transParam.Format = SRA_FMT_RGB565;
	SRA_transParam.BGColor = 0x00;
	...
	for(int i = 0; i < dst_height; i++)
		for(int j = 0; j < dst_width; j++)
			LCD_Buffer[i * LCD_HDOT + j] = SRA_Buffer[i * dst_width + j];
