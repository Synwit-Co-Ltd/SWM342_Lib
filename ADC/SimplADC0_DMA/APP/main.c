...
#define ADC_CNT  	250	// ADC 最多连续转换 256 次
uint16_t ADC_Buffer[ADC_CNT] = {0};
...
int main(void)
{
	...
	ADC_SEQ_initStruct.conv_cnt = ADC_CNT;
	...
	DMA_initStruct.INTEn = DMA_IT_DONE | DMA_IT_DST_PART;
...
void DMA_Handler(void)
{
	...
	if(DMA_CH_INTStat(DMA_CH0, DMA_IT_DST_PART))
	{
		DMA_CH_INTClr(DMA_CH0, DMA_IT_DST_PART);
		
		CirBuf_Write(&CirBuf, &ADC_Buffer[0], ADC_CNT/2);
	}
	else if(DMA_CH_INTStat(DMA_CH0, DMA_IT_DONE))
	{
		DMA_CH_INTClr(DMA_CH0, DMA_IT_DONE);
		
		CirBuf_Write(&CirBuf, &ADC_Buffer[ADC_CNT/2], ADC_CNT - ADC_CNT/2);
	}
}
